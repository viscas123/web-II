...
// Função auxiliar para detectar login do cliente (verifica vários nomes comuns de sessão)
...
<head>
    ...
    <style>
        ...
        .acoes-inline { display:inline-block; vertical-align:middle; }
    </style>
</head>
<body>
<header>
...
<?php if (!empty($_SESSION['carrinho'])): ?>
    ...
    <?php else: ?>
        ...
        <!-- Botões de cancelar e finalizar (separados em forms para evitar aninhamento de forms). -->
        <div style="margin-top: 15px;">
            <form method="post" action="carrinho.php" class="acoes-inline">
                <button type="submit" name="cancelar" style="background:#c85c5a; color:white; padding:10px 15px; border:none; border-radius:8px;">Cancelar Compra</button>
            </form>

            <?php if (cliente_logado()): ?>
                <!-- Cliente logado: envia um POST para pedido_confirmado.php -->
                <form method="post" action="pedido_confirmado.php" class="acoes-inline" style="margin-left:10px;">
                    <button type="submit" name="finalizar" style="padding:10px 20px; background-color:#5D3A6A; color:white; border-radius:8px; border:none; font-weight:bold;">Finalizar Compra</button>
                </form>
            <?php else: ?>
                <!-- Não logado: mantém o comportamento atual (vai para tela de cadastro) -->
